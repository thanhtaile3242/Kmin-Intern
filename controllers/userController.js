...
export const handleSignUp = async (req, res) => {
    ...
    // For generate a uuid key for a new user
    const SECRET_UUID = process.env.SECRET_UUID;
    try {
        ...
        const saltRounds = parseInt(process.env.SALTPASSWORD, 10);
...
export const handleSignIn = async (req, res) => {
    try {
        ...
        const maxAttempts = parseInt(process.env.MAXATTEMPTS, 10);
        const lockoutTime = parseInt(process.env.LOCKOUTTIME, 10);
        ...
        if (isMatch) {
            ...
            const access_token = jwt.sign(payload, process.env.ACCESSTOKENKEY, {
                expiresIn: process.env.EXPIREACCESSTOKEN,
            });
            const refresh_token = jwt.sign(
                payload,
                process.env.REFRESHTOKENKEY,
                {
                    expiresIn: process.env.EXPIREREFRESHTOKEN,
                }
            );
    ...
    try {
        ...
        if (user) {
            ...
            const resetPasswordToken = jwt.sign(
                payload,
                process.env.RESETPASSWORDTOKEN,
                {
                    expiresIn: process.env.EXPIRERESETPASSWORD,
                }
            );
